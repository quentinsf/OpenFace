	...
	while (true) // this is not a for loop as we might also be reading from a webcam
	{
		...
		while (!rgb_image.empty()) // this is not a for loop as we might also be reading from a webcam
		{
			...
			if (detection_success) {	
				...
				globalStream << "GLOBAL";
				for (int i = 0; i < face_model.params_global.rows; i++) {
					globalStream << " " << face_model.params_global[i];
				}
				...
				localStream << "LOCAL";
				for (int i = 0; i < face_model.params_local.rows; i++) {
					localStream << " " << face_model.params_local[i][0];
				}
